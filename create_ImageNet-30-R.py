...
def frost(x, severity=1):
    ...
    frost = cv2.resize(frost, (0, 0), fx=0.8, fy=0.8)
...
def augment(image, select):
    if select == 1:
        ...
        resize_cropper = T.RandomCrop(size=(180, 180))
        resized = T.Resize(size=(224, 224))(resize_cropper(image))
        return resized
...
os.makedirs("ImageNet-30-R", exist_ok=True)
test_transforms = T.Compose([T.Resize((224, 224)), T.ToTensor()])
test_data = dset.ImageFolder('imagenet30/one_class_test', transform=test_transforms)
...
for i in range(1, 6):
    ...
    for img, label in test_loader:
        labels_a.append(label.squeeze())
        imgnet_a.append(np.uint8(augment(convert_img(img.squeeze()), i)))
        

    np.save('ImageNet-30-R/' + augmentations[i-1] + '.npy', np.array(imgnet_a).astype(np.uint8).reshape((-1, 224, 224, 3)))
    np.save('ImageNet-30-R/labels-A.npy', np.array(labels_a).astype(np.uint8))
...
for method_name in d.keys():
    ...
    for severity in range(1,6):
        ...
        for img, label in test_loader:
            labels_c.append(label.squeeze())
            imgnet_c.append(np.uint8(corruption(convert_img(img.squeeze()))))

    np.save('ImageNet-30-R/' + d[method_name].__name__ + '.npy', np.array(imgnet_c).astype(np.uint8).reshape((-1, 224, 224, 3)))

    np.save('ImageNet-30-R/labels-C.npy', np.array(labels_c).astype(np.uint8))