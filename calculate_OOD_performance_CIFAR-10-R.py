...
def read_features(aug, corrup, model, o_dim):
    train_features = np.load("CIFAR-10-R_features/features_" + model + "_train.npy").reshape(50000, o_dim)
    train_labels = np.load("cifar10-R-labels_train.npy").squeeze()

    test_features = np.load("CIFAR-10-R_features/features_" + model + "_test.npy").reshape(10000, o_dim)
    test_labels = np.load("cifar10-R-labels_test.npy").squeeze()
    ...
    if aug == True:
        ...
        for c in near_in:
            near_in_features.append(np.load("CIFAR-10-R_features/features_" + model + '_' + c).reshape(10000, o_dim))
            near_in_labels.append(np.load("cifar10-R-labels_test.npy").reshape(10000,))
    if corrup == True:
        ...
        for c in near_in:
            temp = np.load("CIFAR-10-R_features/features_" + model + '_' + c).reshape(50000, o_dim)
