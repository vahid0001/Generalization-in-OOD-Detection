...
os.makedirs("CIFAR-10-R", exist_ok=True)
test_data = dset.CIFAR10("./", download=True, train=False)
...
for i in range(1, 6):
    ...
    np.save('CIFAR-10-R/' + augmentations[i-1] + '.npy', np.array(cifar_a).astype(np.uint8))
    np.save('CIFAR-10-R/labels-A.npy', np.array(labels_a).astype(np.uint8))
...
for method_name in d.keys():
    ...
    np.save('CIFAR-10-R/' + d[method_name].__name__ + '.npy', np.array(cifar_c).astype(np.uint8))

    np.save('CIFAR-10-R/labels-C.npy', np.array(labels_c).astype(np.uint8))